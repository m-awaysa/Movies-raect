...
const MyRating = ({ currentLngCode, t }) => {
  ...
  const changeToMovie = () => {
    setPathName(`/myRating`)
  }

  const changeToTv = () => {
    setPathName(`/ratedTvShows`)
  }
  const changeToTvEpisode = () => {
    setPathName(`/ratedTvEpisode`)
  }
            ...
            <div className="row">
              <div className="col-12 navbar-expand-sm">
                ...
                <div className='d-flex flex-column'>
                  <h2 className={`content__title ` + t('ms_me_auto')}>{t('watchlist')}</h2>
                ...
                <div className={`collapse navbar-collapse ` + t('flex_end_or_start')} id="watchListContent">
                  <ul className={`navbar-nav content__tabs ` + t('flex_end_or_start')} id="content__tabs" role="tablist">
                    <li onClick={changeToMovie} className="nav-item">
                      <Link className={"nav-link " + ((pathName === '/myRating') ? "active text-danger" : " ")} to="">{t('movies')}</Link>
                    </li>
                    <li onClick={changeToTv} className="nav-item">
                      <Link className={"nav-link " + ((pathName === '/ratedTvShows') ? "active text-danger" : " ")} to="ratedTvShows" >{t('tvshows')}</Link>
                    </li>
                    <li onClick={changeToTvEpisode} className="nav-item">
                      <Link className={"nav-link " + ((pathName === '/ratedTvEpisode') ? "active text-danger" : " ")} to="ratedTvEpisode" >{t('episodes')}</Link>
                    </li>
                  </ul>
                </div>
