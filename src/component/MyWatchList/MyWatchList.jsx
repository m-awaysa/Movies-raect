...
const MyWatchList = ({ currentLngCode, t }) => {

  const [pathName, setPathName] = useState('/movieWatchlist');



  const changeToMovie = () => {
    setPathName(`/movieWatchlist`)
  }

  const changeToTv = () => {
    setPathName(`/tvWatchList`)
  }
  ...
  return (
    <>
      <PageTitle title={t('WatchList')} startPoint={t('home')} />
                  ...
                  <div className={`collapse navbar-collapse ` + t('flex_end_or_start')} id="watchListContent">
                    <ul className={`navbar-nav content__tabs ` + t('flex_end_or_start')} id="content__tabs" role="tablist">
                      <li onClick={changeToMovie} className="nav-item">
                        <Link className={"nav-link " + ((pathName === '/movieWatchlist') ? "active text-danger p-1" : " ")} to="">{t('movies')}</Link>
                      </li>
                      <li onClick={changeToTv} className="nav-item">
                        <Link className={"nav-link " + ((pathName === '/tvWatchList') ? "active p-1 text-danger" : " ")} to="tvWatchList" >{t('tvshows')}</Link>
                      </li>
                    </ul>
                  </div>
