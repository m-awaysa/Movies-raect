...
const RatingCard = ({deleteRating, episode_number = 1, season_number = 1,showId = null, vote_average, poster_path, name, width = '', id, type, classes = '' }) => {
    const [url, setUrl] = useState(`/${type}/${id}`);
    const [tv, setTv] = useState('');


    const getEpisodeURL = async () => {
        if (showId) {
            const { data } = await axios.get(`https://api.themoviedb.org/3/tv/${showId}?api_key=352f716e969ef97191556e88ef26f893&language=en-US`)
         
            setTv(data.name)
            setUrl(`/tv/${data.id}/season/${season_number}/episode/${episode_number}`)
        }
    }
        ...
        <>
            ...
            <div className="col-6 col-sm-4 col-lg-3 col-xl-2"  >
                <div className="movie_card">
                    <svg 
                    onClick={type ==='episode'?()=>deleteRating(showId,season_number,episode_number):()=>deleteRating(id)}
                    ...
                    <Link to={url}>
                        ...
                        <div className="movie_card__content">
                            {showId ? <h3 className="movie_card__title mb-0"><a href="#" className={classes}>
                                {tv}<br />{name}
                            </a></h3>
